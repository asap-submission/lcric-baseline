...
class TQN(nn.Module):
    ...
    def __init__(self, args,first_channel=3,features_out =False,gating=False,SUFB=False,mode='train'):
        ...
        if not self.features_out:
            ...
            # Decoder Layers
            self.H = args.H 
            decoder_layer = TransformerDecoderLayer(self.d_model, args.H, 256,
            ...
            self.classifier_q1 = nn.Linear(self.d_model, args.q1_att)
            self.classifier_q2 = nn.Linear(self.d_model, args.q2_att)
            self.classifier_q3 = nn.Linear(self.d_model, args.q3_att)
            self.classifier_q4 = nn.Linear(self.d_model, args.q4_att)
    ...
    def forward(self, input):
        ...
        # x = x[:, :, :, :224, :224]
        x = self.block1(x)
        # print(x.device)     #### For seeing the device on which the tensor is (confirm multi-gpu support)
        x = self.block2(x)
        x = self.block3(x)
        x = self.block4(x)
        x = self.block5(x)
        ...
        if self.features_out: 
            ...
            x1 = self.classifier_q1(out[0])
            x2 = self.classifier_q2(out[1])
            x3 = self.classifier_q3(out[2])
            x4 = self.classifier_q4(out[4])
            x_cls = self.cls_classifier(out[-1])

            # pdb.set_trace()
            return x1, x2, x3, x4, x_cls
